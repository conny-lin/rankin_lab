classdef rmANOVATWR
   ...
   methods
        ...
        function posthoc_tapbygroup = get.posthoc_tapbygroup(obj)
            D = obj.txtcontent; % get text file
            % get index
            iProb = find(regexpcellout(D,'RevFreq'));
            iDur = find(regexpcellout(D,'RevDur'));
            iSpeed = find(regexpcellout(D,'RevSpeed'));
            % probability
            fulltxt = D(iProb+1:iSpeed-1);
            i = find(regexpcellout(fulltxt,'^Posthoc[(]Tukey[)]tap'));
            fulltxt = fulltxt(i+1:end);
            b = regexpcellout(fulltxt,'(*)|(, )','split');
            T = table;
