function MWTSet = Dance_ShaneSpark3(pMWTS,varargin)
%% INFORMATION
...
%% EXCLUSION: TOTAL N FOR FIRST TAP IS LESS THAN analysisNLimit
MWTSet.analysisNLimit = analysisNLimit;
d = MWTSet.Data.Raw.N.Reversed(1,:);
i = d >= analysisNLimit;
% reporting
mwtfn = MWTSet.Data.Raw.MWTfn(~i);
fprintf('\n\nRemove these plates have less than %d worms detected at first tap from analysis:\n',analysisNLimit);
disp(char(mwtfn));
D = MWTSet.Data.Raw;
B = struct;
Bad = struct;
...
MWTSet.Data.Raw = B;
MWTSet.Data.Raw_lowN = Bad;
% report
p = B.pMWT;
[p,fmwt] = cellfun(@fileparts,p,'UniformOutput',0);
[p,fg] = cellfun(@fileparts,p,'UniformOutput',0);
[~,fe] = cellfun(@fileparts,p,'UniformOutput',0);
fprintf('\nPlates with more than %d worms responding to first tap:\n',analysisNLimit);
tabulate(fg)
...
for g = 1:numel(gnames)
    ...
    T = table;
    T.mwt = D1.MWTplateID;
    fnames(ismember(fnames,'MWTplateID')) = [];
