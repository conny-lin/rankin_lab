function [figure1,plateSumm,savename,Tog,MWTsourceID,recordTime,D] = rasterPlot_colorSpeed(path,start,finish,varargin)
...
cleanup = 1;
masterfile = 0;  % do not save master file
vararginProcessor;
...
convertTrinityDat2Mat(pMWT,cleanup);
...
for x = 1:numel(pMWT)
    ...
    if masterfile == 1 % if directed to save master file
        D.data{x} = d;
    end
