function MWTSet = Dance_AlcoholSTH_v1707(pMWT,pSave,pshanespark,varargin)
%% INFORMATION
...
displayopt = false; % 20170821-changed to false
bypass = 'chor';
% pshanespark = '';
% VarIn = varargin;
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
[MWTSet,pSave] = DanceM_MWTSetStd_v1707(pMWT,funpath,varargin,pSave);
% -------------------------------------------------------------------------
%%                          CHOR | 20170822
% -------------------------------------------------------------------------
% if ismember('chor',bypass)
...
%     % get TRV
%     D = import_trv_table2(pMWT,MWTSet.MWTDB,'displayopt',displayopt); % get trv data
% end
% -------------------------------------------------------------------------
%%                          TRANSFORM DATA | 20170821
% -------------------------------------------------------------------------% -------------------------------------------------------------------------
% if ~isempty(pshanespark) && strcmp(bypass,'chor')
% end
% general variables
load(pshanespark);
MWTDB = MWTSet.MWTDB;
...
D = outerjoin(MWTDB(:,mwtdbvar),D,'type','right','MergeKeys',1);
% translate rx
D = translate_rx2alcohol(D);
% store in output MWTSet
MWTSet.Raw = D;
% -------------------------------------------------------------------------
%%                          CALCULATIONS
% -------------------------------------------------------------------------
% calculate experiment means | 20170821
% -------------------------------------------------------------------------
D = MWTSet.Raw;
D = grpstats(D, {'expname','groupname','tap','strain','dose'},{'mean'},'DataVars',msrlist);
% take out mean
fn = D.Properties.VariableNames;
fnfix = regexprep(fn,'mean_','');
D.Properties.VariableNames = fnfix;
% store data
MWTSet.Data.byExp.Raw = D;
% -------------------------------------------------------------------------
%% calculate percent 0mM control | 20170822
% -------------------------------------------------------------------------
D = MWTSet.Data.byExp.Raw;
matchcolnames = {'expname','strain','tap'};
% match alchohol groups with control
T = unique(D(:,matchcolnames),'rows');
% find controls
i = ismember(D.dose,'0mM');
D0 = D(i,:);
D4 = D(~i,:);
% find number of doses
doselist = unique(D4.dose);
% get row index
[i,rj] = ismember(D0(:,matchcolnames),D4(:,matchcolnames),'rows');
% divide
A = D4;
for msri = 1:numel(msrlist)
    msr = msrlist{msri}; % get msr
    % create empty 
    C = nan(size(D4,1),1);
    % put in index
    C(rj) = D0.(msr);
...
MWTSet.Data.byExp.pctCtrl = A;
% -------------------------------------------------------------------------
% get mean per strain
% -------------------------------------------------------------------------
D = grpstats(A, {'strain','groupname','tap','dose'},{'numel','mean','sem'},'DataVars',msrlist);
% take out mean
fn = D.Properties.VariableNames;
fnfix = regexprep(fn,'mean_','');
D.Properties.VariableNames = fnfix;
% store in ouptut
MWTSet.Data.byExp.byGroup = D;

%%                       GRAPH | 20170822 
% -------------------------------------------------------------------------
%% transform to graphic
D = MWTSet.Data.byExp.byGroup;
gu = unique(D(:,{'groupname'}));
gu = table2cell(gu);
gu = sortN2first(gu,gu);
tapsu = unique(D.tap);
S = struct;
for msri = 1:numel(msrlist)
    msr = msrlist{msri};
    G.groupname = gu;
    G.X = repmat(tapsu,1,numel(gu));
    for gi = 1:numel(gu)
        g = gu{gi};
        i = ismember(D.groupname,g);
        G.N(:,gi) = D.(sprintf('numel_%s',msr))(i,:);
        G.Y(:,gi) = D.(sprintf('%s',msr))(i,:);
        G.E(:,gi) = D.(sprintf('sem_%s',msr))(i,:);
    end
    S.(msr) = G;
end
MWTSet.Graph.pctCtrlbyExpbyGroup = S;
% -------------------------------------------------------------------------
%%                          HAB CURVE | 20170822
% -------------------------------------------------------------------------
% MWTSet = Graph_HabCurve_v1707(MWTSet,msrlist);
% Graph -------------------------------------------------------------------
DataG = MWTSet.Graph.pctCtrlbyExpbyGroup;
for msri = 1:numel(msrlist)
    msr = msrlist{msri};
    gn = DataG.(msr).groupname;
    y = DataG.(msr).Y;
    x = DataG.(msr).X;
    e = DataG.(msr).E;
    Graph_CurveSTH(x,y,e,gn,msr,pSave)
end
%--------------------------------------------------------------------------
