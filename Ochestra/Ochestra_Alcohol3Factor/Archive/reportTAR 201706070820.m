...
if pv < pvsig
    s = sprintf('had different %s compared to the wildtype 0mM group',rspName);
    s = sprintf('%s (curve, %s, pairwise comparison, %s)',s,pvt,posthocstr);
    wt0mt0sig = true;
else
    s = sprintf('had similar %s compared to the wildtype 0mM group',rspName);
    wt0mt0sig = false;
...
txt = sprintf('%s In the presence of ethanol, wildtype',txt);
D = A.posthoc_groups;
...
if mutsig 
    ...
    posthocstr = genPosthocTapbyGStr(A,a{1},a{2},pvlim,pvsig);
    % constrct text
    s = sprintf('Comparison between the wildtype 400mM and the %s 400mM group showed that',genotype);
    if pv < pvsig
...
if ~mutsig 
    ...
    posthocstr = genPosthocTapbyGStr(A,a{1},a{2},pvlim,pvsig);
    % constrct text
    s = sprintf('Comparing to the wildtype 0mM group, %s on ethanol',genotype);
    if pv < pvsig
...
if ~mutsig && wtsig && wt0mt0sig
    ...
    posthocstr = genPosthocTapbyGStr(A,a{1},a{2},pvlim,pvsig);
    % constrct text
    s = sprintf('In fact, comparing to the wildtype 0mM group, the %s 0mM group',genotype);
    if pv < pvsig
        s = sprintf('%s had different %s',s,rspName);
        s = sprintf('%s (curve, %s, pairwise comparison, %s)',s,pvt,posthocstr);
    else
