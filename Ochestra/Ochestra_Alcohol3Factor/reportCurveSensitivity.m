function txt = reportCurveSensitivity(curveData,CS)
...
strain = curveData.strain; % get strain name
genotype = curveData.genotype; % get genotype
[anovatxt,T] = anova(CS); % get ANOVA info
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% v1 = D.pct('N2');
txt = sprintf('%s %s showed a %.0f%%',txt,controlname,abs(v1));
if v1<0
...
if anovapv<pvsig
    ...
    txt = sprintf('%s, representing a %.0f%% %s in ethanol''s effect on body curve compared to %s',txt,d,dirtext,controlname);
end
% ---------------------
