...
%% ACCELERATION
% pSave_Acc = create_savefolder(fullfile(pSave,'TAR')); % create save folder
pData_TAR = Dance_rType2(pMWT,'genotype',genotype); % rType
return


%% REVERSAL
pSave_TWR = create_savefolder(fullfile(pSave,'TWR')); % create save folder
Dance_ShaneSpark5(pMWT,pSave_TWR);



%% INITIAL SENSITIVITY 
pSave_Sen = create_savefolder(fullfile(pSave,'Etoh sensitivity')); % create save folder
Dance_InitialEtohSensitivityPct(pMWT,'pSave',pSave_Sen); % run initial sensitivity
