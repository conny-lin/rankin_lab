function [pMWTA,fval,chorscript] = chormaster3(option, pMWTS,pAnalysis,pData,chorstatus)
%% UPDATES
% 20151016
...
switch chorstatus
    case 'old'
        ...
        val = false(size(pMWTA,1),numel(fval));
        for mi = 1:numel(pMWTA)
            for fvali = 1:numel(fval)
                if isdir(pMWTA{mi}) == 1
                    a = dircontent(pMWTA{mi},fval{fvali});
                    if numel(a) > 0
                        val(mi,fvali) = true;
                    end
                end
        ...
        pMWTcD = pMWTA(sum(val,2) ~= numel(fval));
        pMWTcS = regexprep(pMWTcD,pAnalysis,pData);
