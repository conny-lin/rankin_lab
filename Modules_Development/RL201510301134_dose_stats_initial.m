...
for gi = 1:numel(pG)
    % get pre plate data
    [~,pRaster] = dircontent(pG{gi},rasterName);
    a = dlmread(char(pRaster));
...
%% [suspend] summarize tap response
% prefixName = 'tapResponse';
% 
% % find out tap time for each plate
% formatSpec = '%5s%3s%2s%7s%7s%6s%2s%2s%s%[^\n\r]';
% tapTime = nan(tapNExpected,size(plateInfo,1));
% for x = 1:size(plateInfo,1)
%     % find path to .dat file
%     pM = sprintf('%s/%s/%s/%s',pAnalysis,plateInfo.expname{x},plateInfo.groupname{x}, plateInfo.mwtname{x});
%     [~,f] = fileparts(pM);
%     [~,p] = dircontent(pM,'*.dat');
%     if numel(p) == 0
...
%     else
%         % exclude filenames
%         p(regexpcellout(p,'(tapN_30)')) = [];
%         % find the .dat file with shortest name
%         n = cellfun(@numel,p);
%         p = p(n == min(n)); 
%         if numel(p)> 1; 
...
%     end
% end
% % check plates with no tap record
% if sum(sum(isnan(tapTime),1)) > 1
%     error('some plates have no tap records');
% end
% 



%% [suspend] calculate raster plot with tap aligned in middle
