...
for gi = 1:numel(pG)
    ...
    %% summarize response shift to pause 
    % add dir after if no change as same
    RS1 = tDirChange(:,3);
    RS2 = tDirChange(:,4);
    RS2(isnan(RS2)) = RS1(isnan(RS2));
     
    T(gi,1) = sum(RS1==1 & RS2==0); %for>p
    T(gi,2) = sum(RS1==-1 & RS2==0); % rev>p
    T(gi,3) = sum(RS1==1 & RS2==-1); % for>rev
    T(gi,4) = sum(RS1==-1 & RS2==1); % rev>for
    T(gi,5) = sum(RS1==0 & RS2==1); % p>for
    T(gi,6) = sum(RS1==0 & RS2==-1); % p>rev
    T(gi,7) = sum(RS1 == RS2); % no change
    T(gi,8) = size(tDirChange,1); %total
    G{gi,1} = fG;
       

end

G = cell2table(G,'VariableNames',{'gname'});
T = array2table(T,'VariableNames',{'fp','rp','fr','rf','pf','pr','no_change','total'});
G = [G T];

fn = sprintf('%s/tapResponse_%d_%d/ResponseShiftProb.csv',pSave,start,finish);
writetable(G,fn);
return
