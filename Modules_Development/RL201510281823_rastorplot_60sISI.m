...
%     5 = only experiments containing all groupSearchTerm, and only groups
%     specified in search
searchType = 5;
% paths
...
switch searchType
    ...
    case 5 % only experiments with all groups found
        i = regexpcellout(fG,groupSearchTerm);
        pG = pG(i); fG = fG(i);
        % get experiments containing specified group names
        [pE,fG] = cellfun(@fileparts,pG,'UniformOutput',0);
...
for gi = 1:numel(fGU)
    ...
    for ti = 1:size(timePairs,1)
        ...
        % run raster plot
        [figure1,fn,savename] = rasterPlot_colorSpeed(pathGrouped{gi},timePairs(ti,1),timePairs(ti,2),'NWORMS',Inf,'InputType',2);
        % create save folder
