...
%% paths & settings
% Input
pHome = '/Users/connylin/Dropbox/Lab/MWT_Analysis_ByExp/Dose_10sISI/Stats';
timelist = [95:10:395];
alphaValue = 0.05;
posttestname = 'bonferroni';

% exp info path
pSave = [pHome,'/tapResponseSummary']; if isdir(pSave) == 0; mkdir(pSave); end
...
%% get graph variables
fn_stats = {'ResponseShiftDur_stats.csv','ResponseShiftLatency_stats.csv'};

for fnsi = 1:numel(fn_stats)
    A = [];
    for ti = 1:numel(timelist)-1
        D = readtable(sprintf('%s/tapResponse_%d_%d/%s',pHome,timelist(ti), timelist(ti+1),fn_stats{fnsi}));
        A = [A;[timelist(ti) ti D.mean' D.SE' D.N']];
    end
    a = cellfun(@strcat,D.groupname,cellfunexpr(D.groupname,'_N'),'UniformOutput',0);
    b = cellfun(@strcat,D.groupname,cellfunexpr(D.groupname,'_SE'),'UniformOutput',0);
    vname = [{'start_time','tap'} D.groupname' b' a'];
    T = array2table(A,'VariableNames',vname);
    
    % save
    a = fn_stats{fnsi}(1:strfind(fn_stats{fnsi},'.')-1);
    fn = sprintf('%s/%sSummary.csv',pSave,a);
    writetable(T,fn);
end
...
for x = 1:size(plateInfo,1)
    p = sprintf('%s/%s/%s/%s',pAnalysis,plateInfo.expname{x},plateInfo.groupname{x}, plateInfo.mwtname{x});
    trinitySummary(p,start,finish,'NWORMS',NWORMS,'frameInt',frameInt);
end
