...
fprintf('MWT files included: %d\n\n',numel(fMWT));



%% examine number of MWT files for chor
pMWTA = regexprep(regexprep(pMWT,'(.zip)',''),pData,pAnalysis);
f = cellfun(@dircontent,pMWTA,cellfunexpr(pMWTA,chorfileName),'UniformOutput',0);
n = sum(cellfun(@numel,f) == 0);
fprintf('MWT without %s: %d/%d\n',chorfileName,n,numel(pMWTA))
opt = input('[1] Proceed, [any other key] abort');
if opt ~=1; display 'program aborted'; return; end
