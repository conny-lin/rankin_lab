...
addpath('/Users/connylin/Dropbox/Lab/Code/Library/Modules/MWTDatabase');
...
pOut = '/Volumes/COBOLT/unzip';
pA = pOut;
pKeep = '/Volumes/COBOLT/Keep';
FileLimitN = 1000;
...
pEAZip(i) = [];
fEAZip(i) = [];
if FileLimitN < numel(pEAZip)
    rowN = FileLimitN;
else
    rowN = numel(pEAZip);
end
for x = 1:rowN
    fprintf('%d/%d processed, total source files %d\n',x,rowN,numel(pEAZip));
    pz = pEAZip{x};
    ...
    unzip(pz);
    cd(pStd)
    tsfNewFiles2SameMWTFolder(pMWTD,pOut,pKeep)
    rmdir(pOut,'s');
    delete(pz)
end
